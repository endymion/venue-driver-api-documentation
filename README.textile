...
Flyers are a nested resource under the event resource.  They are not included in the event list returned by /venues/:venue_id/events.xml, but you can obtain them through the show action for the event resource.  For example, this will return the list of flyer images for the event 5678 at the venue 1234:
